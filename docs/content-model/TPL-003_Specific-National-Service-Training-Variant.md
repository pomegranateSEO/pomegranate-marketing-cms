# TPL-003: National Specific Service Page — Training Variant

**Template:** 3 of 13 (Training Variant)
**URL Pattern:** `/seo-training/[course-slug]/` — e.g. `/seo-training/advanced-seo-mastery/`, `/seo-training/seo-foundations/`
**Purpose:** Primary commercial page for individual training courses at national level. Covers all training/course offerings. The main conversion-driving page for each training service offering.
**Schema:** WebPage, Service (EducationalService), Course, FAQPage (conditional), BreadcrumbList, Organization (ref)
...
### Training Service Variant — Complete Schema Specification

When this template is used for a training/course page (e.g. `/seo-training/[course-slug]/`), the schema emits both a Service (as mainEntity) AND a separate Course entity. The two exist independently in the @graph with no explicit link between them — both are referenced from WebPage.

**@graph Structure for Training Variant:**

| @graph Item | @type | @id | Notes |
| :--- | :--- | :--- | :--- |
| **WebPage** | WebPage | `{pageUrl}/` | Leaf-level course page |
| **Service** | Service | `{pageUrl}/#service` | The training service offering, with EducationalService type and Offer |
| **Course** | Course | `{pageUrl}/#course` | The course curriculum definition (separate entity) |
| **FAQPage** | FAQPage | `{pageUrl}/#faq` | Conditional — only when FAQ items exist and FAQ Schema Toggle is true |
| **BreadcrumbList** | BreadcrumbList | `{pageUrl}/#breadcrumb` | Home → SEO Training → [Course Name] |
| **ImageObject** | ImageObject | `{pageUrl}/#primaryimage` | Conditional — only if OG image exists |

#### Service Fields (Training Variant)

| Field Name | Source | Required? | Notes |
| :--- | :--- | :--- | :--- |
| **@type** | Fixed: "Service" | Yes | Service is the mainEntity for the WebPage |
| **@id** | `{pageUrl}/#service` | Yes | |
| **name** | `pages.service_name` | Yes | e.g. "SEO Training Course" |
| **description** | `pages.service_description` | Yes | |
| **serviceType** | Fixed: "EducationalService" | Yes | Training services always use EducationalService |
| **additionalType** | Fixed: `https://www.wikidata.org/wiki/Q918385` | Yes | Wikidata: Training |
| **provider** | @id ref → `#organization` | Yes | |
| **areaServed** | Country or Place | No | e.g. `{ "@type": "Country", "name": "United Kingdom" }` |
| **subjectOf** | @id ref → `{pageUrl}/#faq` | No | Only when FAQPage exists |
| **offers** | Offer object | Yes | Always present — course pricing with "Contact for pricing or group booking rates" description |

#### Offer Fields (Training Variant)

| Field Name | Source | Required? | Notes |
| :--- | :--- | :--- | :--- |
| **@type** | Fixed: "Offer" | Yes | |
| **priceCurrency** | Fixed: "GBP" | Yes | |
| **price** | `course.price_value` or "0" | Yes | Set to actual price if available, or "0" if custom/contact |
| **description** | Fixed: "Contact for pricing or group booking rates" | Yes | Standard text for training courses |
| **url** | `{pageUrl}/` | Yes | Link to course page |

#### Course Fields (Training Variant)

A separate Course entity providing structured curriculum metadata. This is **independent** of the Service — it documents what the course teaches, not how the training is offered.

| Field Name | Source | Required? | Notes |
| :--- | :--- | :--- | :--- |
| **@type** | Fixed: "Course" | Yes | |
| **@id** | `{pageUrl}/#course` | Yes | |
| **name** | `course.name` | Yes | e.g. "Advanced SEO Training" |
| **description** | `course.description` | Yes | What the course covers and expected outcomes |
| **courseMode** | `course.mode` | Yes | Possible values: "OnlineOnly", "InPersonOnly", "MixedMode", "Hybrid" |
| **educationalLevel** | `course.level` | Yes | Possible values: "Beginner", "Intermediate", "Advanced" |
| **timeRequired** | `course.duration` | No | ISO 8601 duration format — e.g. "PT3D" (3 days), "PT24H" (24 hours), "P4W" (4 weeks) |
| **provider** | @id ref → `#organization` | Yes | Same as Service provider |

#### Training Variant WebPage Fields

The WebPage in training variant adds mentions and about for educational and SEO concepts:

| Field Name | Source | Required? | Notes |
| :--- | :--- | :--- | :--- |
| **about** | Array of Thing objects | Yes | Should include: Training (Q918385), Search engine optimization (Q180711) with both Wikipedia + Wikidata sameAs |
| **mentions** | Array of Thing objects | Yes | Should include: Digital marketing (Q1321787) with both Wikipedia + Wikidata sameAs |

#### FAQPage Fields (Training Variant)

Same as standard FAQPage, but questions should be training-specific:
...
Only emitted when FAQ items exist and FAQ Schema Toggle is true. Does NOT include a `url` field — only `@id` and `mainEntity`.
...
See `/docs/content-model/schema-examples/TPL-003_Specific-National-Service-Training-Variant_schema.jsonld` for the complete working template.
